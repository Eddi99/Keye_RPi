...
from PyQt5.QtCore import Qt, pyqtSignal, pyqtSlot # PyQt6 für Fenstersteuerung und Punktkoordinaten


class GUIApp(QWidget):
	update_frame_signal = pyqtSignal(np.ndarray)  # Signal, das den Frame an GUI sendet
	
	def __init__(self, logic):
		super().__init__()
		self.logic = logic  # Speichert die Referenz zur Entscheidungslogik
        
		self.update_frame_signal.connect(self.update_frame) # Verbinde das Signal mit dem Slot (Thread-safe!)
		self.logic.detector.set_frame_callback(self.update_frame_signal.emit) # Frame-Callback verbindet sich mit Signal-Emitter
	...
	def capture_frame(self):
		...
		cap = cv2.VideoCapture(0)  # Öffnet die Kamera mit Index 0
	...
	def confirm_rois(self):
		...
		threading.Thread(target=self.logic.start_detection, daemon=True).start()
